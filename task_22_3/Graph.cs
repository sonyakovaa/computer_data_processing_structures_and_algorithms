...
namespace graph_1
{
    public class Graph
    {
        private class Node  //вложенный класс для скрытия данных и алгоритмов
        {
            ...
            public void NovSetDijkstr(List<String> towns)
            {
                for (int i = 0; i < Size; i++)
                {
                    nov[i] = true;
        ...
        public void Dijkstr(int v, int b, List<String> towns)
        {
            graph.NovSetDijkstr(towns); //помечаем все вершины графа как непросмотренные
            int[] p;
            ...
            for (int i = 0; i < graph.Size; i++)
            {
                if (i != v && i == b)
                {
                    if (d[i] != int.MaxValue)
                    {
                        ...
                        while (items.Count != 0)
                        {
                            itemsTowns.Add(items.Pop().ToString());
                        }

                        Console.Write("Длина кратчайшие пути от вершины {0} до вершины {1}: ", v, b);
